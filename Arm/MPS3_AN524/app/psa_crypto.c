					...
					size_t *key_len)
{
	...
	printf("Retrieving public key for key #%ld\n", key_id);
	...
	if (status != PSA_SUCCESS) {
		printf("Failed to open persistent key #%ld\n", key_id);
		goto err;
	}
	...
	status = al_psa_status(
		...
		__func__);
	if (status != PSA_SUCCESS) {
		printf("Failed to close persistent key.\n");
		goto err;
	}
					  ...
					  uint8_t *key_data)
{
	...
	printf("Persisting SECP256R1 key as #%ld\n", (uint32_t)key_id);
	...
	if (key_usage & PSA_KEY_USAGE_EXPORT) {
		...
		/* Check key len. */
		if (data_len != key_len) {
			printf("Unexpected number of bytes in exported key.\n");
			goto err;
		}
		...
		if (comp_result != 0) {
			printf("Imported/exported private key mismatch.\n");
			goto err;
		}
					 ...
					 size_t *hash_len)
{
	...
	printf("Calculating SHA-256 hash of value\n");
	...
	status = al_psa_status(psa_hash_setup(&hash_handle, alg),
				   __func__);
	if (status != PSA_SUCCESS) {
		printf("Failed to setup hash op.\n");
		goto err;
	}
	...
	status = al_psa_status(psa_hash_finish(&hash_handle,
				   ...
				   __func__);
	if (status != PSA_SUCCESS) {
		printf("Failed to finalize hash op.\n");
		goto err;
	}
				  ...
				  size_t *sig_len)
{
	...
	status = al_psa_status(
		...
		__func__);
	if (status != PSA_SUCCESS) {
		printf("Failed to sign hash w/persistent key #%ld\n", key_id);
		goto err;
	}
					...
					uint8_t *sig, size_t sig_len)
{
	...
	printf("Verifying signature for SHA-256 hash\n");
...
static psa_status_t crp_dest_key(psa_key_id_t key_id)
{
	...
	status = al_psa_status(
		...
		__func__);
	if (status != PSA_SUCCESS) {
		printf("Failed to destroy a persistent key\n");
		goto err;
	}

	printf("Destroyed persistent key #%ld\n", (uint32_t)key_id);
...
void crp_test(void)
{
	...
	status = crp_hash_payload(msg, strlen((char *) msg),
...
void crp_test_rng(void)
{
	...
	if (status == PSA_SUCCESS) {
		printf("Generating 256 bytes of random data.\n");
		sf_hex_tabulate_16(&crp_fmt, outbuf, 256);
	} else {
		printf("Generating random data failed.\n");
	}
}
