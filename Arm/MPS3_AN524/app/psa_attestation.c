...
psa_status_t att_get_iat(uint8_t *ch_buffer, uint32_t ch_sz,
			 uint8_t *token_buffer, size_t *token_sz)
{
	psa_status_t err = PSA_SUCCESS;
	size_t sys_token_sz;
	...
	printf("att: System IAT size is: %d bytes.\n", sys_token_sz);
	if (err) {
		goto err;
	}

	/* Request the initial attestation token w/the challenge data. */
	printf("att: Requesting IAT with %ld byte challenge.\n", ch_sz);
	err = psa_initial_attest_get_token(
	...
	printf("att: IAT data received: %d bytes.\n", *token_sz);
...
psa_status_t att_test(void)
{
	...
	/* IAT response buffer. */
	size_t iat_sz = ATT_MAX_TOKEN_SIZE;
